...
namespace Компилятор
{
    class SyntaxAnalyzer
    {
        ...
        public static List<string> variables_int = new List<string>(); // Список для хранения переменных типа int
        public static List<string> variables_real = new List<string>(); // Список для хранения переменных типа real
        public static List<string> variables_bool = new List<string>(); // Список для хранения переменных типа boolean
        public static List<string> variables_char = new List<string>(); // Список для хранения переменных типа char
        ...
        {
            bool type_int = false, type_real = false, type_bool = false, type_char = false; // Проверка к переменной какого типа осуществляется присваивание
            //Если переменная существует в списке variables_int, то для неё характерны операции с переменными типа int
            if (variables_int.Contains(LexicalAnalyzer.name))
            {
                type_int = true;
            }
            //Если переменная существует в списке type_real, то для неё характерны операции с переменными типа real
            if (variables_real.Contains(LexicalAnalyzer.name))
            {
                type_real = true;
            }
            //Если переменная существует в списке variables_bool, то для неё характерны операции с переменными типа bool
            if (variables_bool.Contains(LexicalAnalyzer.name))
            {
                type_bool = true;
            }
            //Если переменная существует в списке variables_char, то для неё характерны операции с переменными типа char
            if (variables_char.Contains(LexicalAnalyzer.name))
            {
                type_char = true;
            }
            ...
            {
                if (type_int == false && type_real == false && type_bool == false && type_char == false)
                {
                ...
                while(LexicalAnalyzer.symbol != LexicalAnalyzer.semicolon)
                {
                    ...
                    {
                        neytraz = true;
                        // Если происходит присваивание двух несоответствующих типов данных, выводится ошибка
                        if (type_int == true && variables_int.Contains(LexicalAnalyzer.name) == false)
                        {
                            InputOutput.Error(2, LexicalAnalyzer.token);
                        }
                        if (type_real == true && variables_real.Contains(LexicalAnalyzer.name) == false)
                        {
                            InputOutput.Error(2, LexicalAnalyzer.token);
                        }
                        if (type_bool == true && variables_bool.Contains(LexicalAnalyzer.name) == false)
                        {
                            InputOutput.Error(2, LexicalAnalyzer.token);
                        }
                        if (type_char == true && variables_char.Contains(LexicalAnalyzer.name) == false)
                        {
                            InputOutput.Error(2, LexicalAnalyzer.token);
                        }
                        accept(LexicalAnalyzer.str); 
        ...
        public static void varpart()
        {
            ...
            List<string> vars_type = new List<string>(); // Список для хранения объявляемых переменных при дальнейшем их определении типа.
            do
            {
                variables.Add(LexicalAnalyzer.name); // Запись переменной в список, с переменными
                vars_type.Add(LexicalAnalyzer.name); // Запись переменной в список, для дольнейшей сортировки по типам данных
                accept(LexicalAnalyzer.str);
                ...
                if (LexicalAnalyzer.symbol == LexicalAnalyzer.colon)
                {
                    ...
                    {
                        InputOutput.PrintSymbolCodeInFile();
                        // Определяем тип данных переменных и записываем их в соответствующий список, для проверки правильности операций с ними
                        if(LexicalAnalyzer.symbol == LexicalAnalyzer.integersy)
                        {
                            variables_int.AddRange(vars_type);
                        }
                        if (LexicalAnalyzer.symbol == LexicalAnalyzer.realsy)
                        {
                            variables_real.AddRange(vars_type);
                        }
                        if (LexicalAnalyzer.symbol == LexicalAnalyzer.charsy)
                        {
                            variables_char.AddRange(vars_type);
                        }
                        if (LexicalAnalyzer.symbol == LexicalAnalyzer.boolsy)
                        {
                            variables_bool.AddRange(vars_type);
                        }
                        vars_type.Clear(); // Очищает список с переменными для дальнейшей проверки
                        LexicalAnalyzer.NextSym();
                        accept(LexicalAnalyzer.semicolon);
                    }
                    else
                ...
                else
                {
                    if (LexicalAnalyzer.symbol == LexicalAnalyzer.str)
                    {
                        ...
                        do
                        {
                            ...
                            vars_type.Add(LexicalAnalyzer.name);
                            accept(LexicalAnalyzer.str);
                            count++;
                            if (LexicalAnalyzer.symbol == LexicalAnalyzer.colon)
                            {
                                ...
                                LexicalAnalyzer.symbol == LexicalAnalyzer.boolsy || LexicalAnalyzer.symbol == LexicalAnalyzer.charsy)
                                {
                                    ...
                                    vars_type.Clear(); // Очищает список для дальнейшей проверки переменных
                                    LexicalAnalyzer.NextSym();
                                    if (LexicalAnalyzer.symbol != LexicalAnalyzer.rightpar)
                                        accept(LexicalAnalyzer.semicolon);
                                }
