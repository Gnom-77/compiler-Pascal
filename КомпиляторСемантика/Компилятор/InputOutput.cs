...
namespace Компилятор
{
    ...
    class InputOutput
    {
        ...
        public static int begin_score; // Подсчёт ключевого слова begin
        public static int end_score; // Подсчёт ключевого слова end
        public static bool door_for_end = true; // Переход к слову end
        public static bool door_for_begin = true; // Переход к слову begin
        ...
        public static void PrintSymbolCodeInFile()
        {
            character_codes.Write(LexicalAnalyzer.symbol + " ");
        }
        ...
        static void End()
        {
            ...
            if (errCount == 0 && begin_score == end_score)
            {
            ...
            else
            {
                ...
                {
                    Console.WriteLine("Компиляция завершена с ошибкой, ожидалось ключевое слово end");
                    export.WriteLine("Компиляция завершена с ошибкой, ожидалось ключевое слово end");
                    errCount += Convert.ToByte(begin_score - end_score); 
                }
                if (begin_score < end_score)
                {
                    Console.WriteLine("Компиляция завершена с ошибкой, ожидалось ключевое слово begin");
                    export.WriteLine("Компиляция завершена с ошибкой, ожидалось ключевое слово begin");
                    errCount += Convert.ToByte(end_score - begin_score);
                }
                if (errCount < 10)
                {
        ...
        static void ListErrors()
        {
            ...
            foreach (Err item in err)
            {
                ...
                if (errCount < 10)
                {
                    ...
                    {
                        case 84:
                            s += ": ошибка не верное название переменной";
                            break;
                        case 121:
                            s += ": пропущенный символ, ожидалось ')'";
                            break;
                        case 77:
                            s += ": неверное колличество параметров";
                            break;
                        case 79:
                            s += ": неопределён тип данных";
                            break;
                        case 1:
                            s += ": ошибка несуществующая переменная";
                            break;
                        case 2:
                            s += ": ошибка несоответствующие типы данных";
                            break;
                        case 201:
